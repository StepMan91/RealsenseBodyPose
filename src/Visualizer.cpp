...
void Visualizer::drawKeypoint(cv::Mat& image, const Keypoint2D& kpt, const cv::Scalar& color) {
    ...
    cv::circle(image, center, config_.keypointRadius + 1, cv::Scalar(255, 255, 255), -1);
    cv::circle(image, center, config_.keypointRadius, color, -1);
}

void Visualizer::drawBone(cv::Mat& image, const Keypoint2D& kpt1, const Keypoint2D& kpt2,
    ...
    cv::line(image, pt1, pt2, cv::Scalar(255, 255, 255), config_.boneThickness + 2);
    cv::line(image, pt1, pt2, color, config_.boneThickness);
}

void Visualizer::drawBBox(cv::Mat& image, const float* bbox, const cv::Scalar& color) {
...
void Visualizer::drawFPS(cv::Mat& image, double fps) {
    ...
    // Draw text
    cv::putText(image, ss.str(), cv::Point(15, 35), cv::FONT_HERSHEY_SIMPLEX,
               0.7, cv::Scalar(0, 255, 0), 2);
}

void Visualizer::draw(cv::Mat& image, const std::vector<Skeleton>& skeletons, double fps) {
    ...
    for (size_t personIdx = 0; personIdx < skeletons.size(); personIdx++) {
        ...
        if (personIdx == 1) bboxColor = cv::Scalar(255, 0, 0);  // Blue
        if (personIdx == 2) bboxColor = cv::Scalar(0, 0, 255);  // Red
    ...
    // Draw instructions
    cv::putText(image, "Press ESC to quit", cv::Point(10, image.rows - 15),
...
void Visualizer::print3DCoordinates(const std::vector<Skeleton>& skeletons) {
    ...
    for (size_t personIdx = 0; personIdx < skeletons.size(); personIdx++) {
        ...
        std::cout << "\nPerson " << (personIdx + 1) << " (Confidence: " 
                 << std::fixed << std::setprecision(2) << skeleton.overallConfidence << "):\n";
        ...
        for (int jointIdx : majorJoints) {
            ...
            if (kpt3D.isValid()) {
                std::cout << "  " << std::setw(15) << std::left << getJointName(jointIdx) << ": "
                         << "X=" << std::setw(7) << std::fixed << std::setprecision(3) << kpt3D.x << "m  "
                         << "Y=" << std::setw(7) << std::fixed << std::setprecision(3) << kpt3D.y << "m  "
                         << "Z=" << std::setw(7) << std::fixed << std::setprecision(3) << kpt3D.z << "m  "
